...
def submit_node(node, duration, inputs):
            if node.startswith("wordcount_bucketed_"):
        return wordcount_bucketed_synthetic(node, duration, inputs=inputs)
    if node.startswith("reduce_bucket_"):
        return reduce_bucket_synthetic(node, duration, inputs=inputs)
    if node.startswith("merge_outputs_"):
        return merge_outputs_synthetic(node, duration, inputs=inputs)

    raise ValueError(f"Unknown synthetic task type for node: {node}")


def run_parsl_from_workflow(wf):
    futures = {}
    ...
    for node in nx.topological_sort(wf.dag_parsed):
        ...
        futures[node] = submit_node(node, duration, inputs)
...
start = time.time()
    ...
    try:
        ...
        results = run_parsl_from_workflow(workflow)
